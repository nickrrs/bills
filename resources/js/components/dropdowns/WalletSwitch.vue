<template>
    <DropdownMenu v-model:open="isOpen">
        <DropdownMenuTrigger as-child>
            <UiButton
                id="wallet-switch"
                size="sm"
                class="flex w-[193px] items-center justify-between overflow-hidden !rounded-md border border-border bg-card text-foreground hover:bg-accent"
            >
                <LoaderCircle v-if="loading && wallets.length === 0" class="!h-4 !w-4 flex-shrink-0 animate-spin text-muted-foreground" />
                <div v-else class="flex min-w-0 flex-1 items-center gap-x-1 overflow-hidden">
                    <span class="mt-[3px] flex-shrink-0 text-sm font-medium" :style="{ color: currentAccentColor }">@</span>
                    <span class="flex-1 truncate text-left" :class="selectedWalletId ? 'text-foreground' : 'text-muted-foreground'">{{
                        displayWalletName ? `${displayWalletName} wallet` : 'selecionar carteira'
                    }}</span>
                </div>
                <ChevronsUpDown v-if="!isOpen && (!loading || wallets.length > 0)" class="ml-1 !h-4 !w-4 flex-shrink-0 text-muted-foreground" />
                <ChevronsDownUp v-else-if="isOpen && (!loading || wallets.length > 0)" class="ml-1 !h-4 !w-4 flex-shrink-0 text-muted-foreground" />
            </UiButton>
        </DropdownMenuTrigger>
        <DropdownMenuContent class="w-80 !rounded-md border-border bg-popover">
            ...
            <div class="px-2 py-2">
                <div class="relative">
                    <Search class="absolute left-2 top-1/2 !h-4 !w-4 -translate-y-1/2 transform text-muted-foreground" />
                    <input
                        v-model="searchQuery"
                        type="text"
                        placeholder="buscar carteira..."
                        class="w-full rounded-md border border-border bg-card py-2 pl-8 pr-3 text-sm text-foreground placeholder-muted-foreground focus:border-transparent focus:outline-none focus:ring-2"
            ...
            <DropdownMenuGroup class="max-h-[200px] overflow-y-auto">
                ...
                <DropdownMenuItem
                    ...
                    :key="wallet.id"
                    :class="[
                        'cursor-pointer !rounded-md pl-2 text-muted-foreground',
                ...
                >
                    <div class="flex w-full items-center justify-center gap-x-0.5">
                        <span class="mt-[1px] text-sm font-medium" :style="{ color: currentAccentColor }">{{
                            selectedWalletId === wallet.id ? '' : '@'
                        }}</span>
                        <span class="flex-1 text-sm" :class="selectedWalletId === wallet.id ? 'font-bold text-white' : 'text-muted-foreground'">{{
                            wallet.name
                        }}</span>
                        <span v-if="selectedWalletId === wallet.id" class="text-xs font-bold text-white">
                            <Check class="!h-4 !w-4" />
                        </span>
                    </div>
                </DropdownMenuItem>
            ...
            <div v-if="pagination && pagination.last_page > 1" class="flex items-center justify-between border-t border-border px-2 py-2">
                <button
                    @click="goToPreviousPage"
                    :disabled="pagination.current_page === 1"
                    class="flex items-center gap-1 px-2 py-1 text-xs text-muted-foreground hover:text-foreground disabled:cursor-not-allowed disabled:opacity-50"
                >
                    <ChevronLeft class="!h-3 !w-3" />
                    anterior
                </button>
                <span class="text-xs text-muted-foreground"> página {{ pagination.current_page }} de {{ pagination.last_page }} </span>
                <button
                    @click="goToNextPage"
            ...
            <DropdownMenuSeparator class="bg-border" />
            <DropdownMenuGroup>
                <DropdownMenuItem class="cursor-pointer !rounded-md text-muted-foreground hover:!bg-accent" @click="handleAddWallet">
                    <Plus class="!h-4 !w-4" />
                    adicionar carteira
                </DropdownMenuItem>
            </DropdownMenuGroup>
    ...
    <Sheet v-model:open="isDrawerOpen" v-if="!isOnWalletsPage">
        <SheetContent side="right" class="w-full overflow-y-auto border-border bg-card sm:max-w-2xl">
            <SheetHeader>
                <SheetTitle class="text-lg font-semibold text-foreground">criar nova carteira</SheetTitle>
            </SheetHeader>
            <div class="mt-6">
                <CreateWalletForm
...
import { Check, ChevronLeft, ChevronRight, ChevronsDownUp, ChevronsUpDown, LoaderCircle, Plus, Search } from 'lucide-vue-next';
